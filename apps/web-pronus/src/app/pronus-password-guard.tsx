...
function readSession(): PronusAccessProfile | null {
  const raw = window.localStorage.getItem(operatorSessionKey);
  ...
  try {
    return JSON.parse(raw) as PronusAccessProfile;
  } catch {
    return null;
  }
}

export function PronusPasswordGuard() {
  const [session, setSession] = useState<PronusAccessProfile | null>(null);
  ...
  async function submitPassword() {
    ...
    try {
      const response = await fetch(`${getApiUrl()}/pronus-access/password`, {
        body: JSON.stringify({ userId: session.id, newPassword: password }),
      ...
      const nextSession = payload as PronusAccessProfile;
      window.localStorage.setItem(operatorSessionKey, JSON.stringify(nextSession));
      setSession(nextSession);
      window.dispatchEvent(new Event("pronus-session-updated"));
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4 py-6">
      <section className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        <h2 className="text-lg font-semibold text-slate-950">Atualize sua senha PRONUS</h2>
        <p className="mt-2 text-sm text-slate-600">
          Para liberar o painel operacional, escolha uma senha de 6 caracteres com letra, numero e
          caractere especial.
        </p>
